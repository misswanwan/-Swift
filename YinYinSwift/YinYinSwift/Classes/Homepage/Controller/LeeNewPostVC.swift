...
class LeeNewPostVC: LeeBaseVC ,ScrollTabProtocal{

    private let hotPostIdentifier = "hotPostIdentifier"
    
    var refreshCourse : (() -> Void)?
...
extension LeeNewPostVC{
    func requestHotPostList(isLoadMore:Bool){
        NetworkTool.homepagePostListRequest(type: 1, pageNo: self.pageNo, pageSize: self.pageSize, completionHandler: { [weak self]  (dataDic) in
            self!.refreshCourse?()
...
extension LeeNewPostVC:UITableViewDelegate,UITableViewDataSource{
